...
mod tests {
    ...
    #[test]
    fn exposes_expected_name_and_tool_spec() {
        let tool = BashTool;
    ...
    async fn captures_stderr() {
        let result = BashTool
            .invoke(&json!({ "command": "printf 'problem' >&2" }))
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn returns_marker_when_command_has_no_output() {
        let result = BashTool
    ...
    async fn blocks_dangerous_command() {
        let error = BashTool
            .invoke(&json!({ "command": "sudo echo blocked" }))
            .await
            .unwrap_err();
